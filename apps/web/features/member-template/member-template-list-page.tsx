...
import { useDeleteMemberTemplate } from './use-delete-member-template';
import { useMemberTemplates } from './use-member-templates';
...
export const MemberTemplateListPage = () => {
  const query = useMemberTemplates();
  const destroy = useDeleteMemberTemplate();
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className='flex gap-2'>
          <Link href={`/admin/member-templates/${row.original.id || ''}`}>
  ...
  return (
    <>
      <div className='flex justify-center flex-col md:flex-row md:justify-between items-center flex-wrap gap-4 mb-8'>
        ...
        <Link href='/admin/member-templates/create'>
